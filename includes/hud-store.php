...
function alexk_press_job_option_key(string $job_id): string {
  return 'alexk_press_job_' . preg_replace('/[^a-zA-Z0-9_\-]/', '', $job_id);
}

function alexk_press_job_now(): int {
  return time();
}
 ...
 */
function alexk_press_job_create(array $args): array {
  $job_id = $args['job_id'] ?? '';
  if (!$job_id) {
  ...
  $job = [
    ...
    // Timing:
    'started_ts'      => alexk_press_job_now(),
    'last_update_ts'  => alexk_press_job_now(), // MUST update whenever done/errors changes

    // Optional debug/help:
    'last_error'      => '',
  ];

  $key = alexk_press_job_option_key($job_id);
 ...
 * Read job truth.
 */
function alexk_press_job_get(string $job_id): ?array {
  $key = alexk_press_job_option_key($job_id);
 ...
 */
function alexk_press_job_patch(string $job_id, array $patch): ?array {
  ...
  if (!isset($job['last_update_ts']) || !is_int($job['last_update_ts'])) {
    $job['last_update_ts'] = alexk_press_job_now();
  }

  update_option(alexk_press_job_option_key($job_id), $job, false);
  return $job;
}
 ...
 */
function alexk_press_job_tick_success(string $job_id, array $current = []): ?array {
  $job = alexk_press_job_get($job_id);
  if (!$job) return null;
 ...
 */
function alexk_press_job_tick_error(string $job_id, string $error_message, array $current = []): ?array {
  $job = alexk_press_job_get($job_id);
  if (!$job) return null;
 ...
 */
function alexk_press_job_finish(string $job_id, string $status = 'done'): ?array {
  $allowed = ['done', 'canceled', 'error'];
  if (!in_array($status, $allowed, true)) $status = 'done';

  return alexk_press_job_patch($job_id, [
    'status' => $status,
    'last_update_ts' => alexk_press_job_now(),
  ]);
}
